...
int countofDegree(struct node* temp){
	int x=0;
	while(temp!=NULL){
		x+=1;
		temp=temp->right;
...
void WordFrequencyinDes(struct node* root,int count){
	while(root!=NULL){
...
void AddingCategory(struct node* root,char* ch,int x){
	if(x==0){
	if(root->left == NULL){
		struct node* temp=(struct node*)malloc(sizeof(struct node));
		strcpy(temp->data,ch);
	...
	if(root->left == NULL){
		...
		if(strcmp(root->data,ch) == 0){
			if(root->left == NULL){
				...
				freq->right=NULL;
				root->left=freq;
				printf("\nWe have this word in tree therefore Frequency of this word is updated\n\n");
				return;
			}
			else{
		...
		if(strcmp(root->data,ch) == 0){
			if(root->left == NULL){
				...
				char c1=root->left->data[0];// update frequency when it is more than 2 times
				int x=c1-'0';
				x=x+1;
				if(x > frequency){
					frequency=x;
...
printf("1.Input Category\n2.Input word\n3.print\n4.Display Leaf nodes excluding Level 4\n5.Depth of any Node\n6.Degree of any Node\n7.Display the Words in Frequency Descending order\n8.Exit\nEnter Your choice : \n");
        ...
        if(choice==1){
			...
			int p=frequency;
			while(p>0){
			WordFrequencyinDes(root->left,p);
			p--;
			}
			if(wordcheck==0)
		...
		break;
			...
			printf("\nEnter integers between 1 to 8\n\n");
}
else{
	printf("\nEnter valid Datatype(intergers)\n");
	exit(0);
    }
  }
}
